...
//					MarkUtils.pressText(directory+"/"+filePath, watermark);
	...
	public void ueditor() {
		...
		if("config".equals(action)) {
			...
			try {
				String configPath = getRequest().getSession().getServletContext().getRealPath("/") + "\\js\\ueditor\\config.json";
...
//						MarkUtils.pressText(directory+"/"+filePath, watermark);
						...
						returnMsg.put("url", "/downloadData?filePath=/ueditor/"+thisDate+"/"+filePath+"&fileName="+fileName);
...
//						MarkUtils.pressText(directory+"/"+filePath, watermark);
						if(isTmep) {
							...
							returnMsg.put("url", "/downloadData?filePath=/ueditor/"+thisDate+"/"+filePath);
							returnMsg.put("size", newFile.length());
						}
					}
				} catch (Exception e) {
